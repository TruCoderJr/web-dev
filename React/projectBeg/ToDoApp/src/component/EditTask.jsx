...
function  EditTask() {
  ...
  const handleTaskEdit = (eleIdx) => {
    
    const seletedTask = task[eleIdx];
    navigate(`/EditTask/${seletedTask.task}`);
  };
        ...
        <div className="flex flex-wrap w-full bg-gray-100 gap-4 justify-center">
          ...
          {task.map((task, idx) => (
            <div key={idx} className="m-1 ">
              <div className={`lg:w-72 sm:w-full bg-blue-100 px-8 pt-2 pb-4 rounded-lg overflow-hidden text-start relative ${task.isDone?"opacity-50":"" }`}>
