...
function DeleteTask() {
  ...
  const newArr = task.filter((ele, index) => index !== idx);
  setTask(newArr);

  newArr.forEach((task, newIndex) => {
    localStorage.setItem(newIndex, JSON.stringify(task));
  });
};
        ...
        <div className="flex flex-wrap w-full bg-gray-100 gap-4 justify-center">
          ...
          {task.map((task, idx) => (
            <div key={idx} className="m-1 ">
              <div className={`lg:w-72 sm:w-full bg-red-100 px-8 pt-2 pb-4 rounded-lg overflow-hidden text-start relative ${task.isDone?"opacity-50":"" }`}>
                ...
                <button className="inline-flex items-center justify-center border-0 py-2 px-6 focus:outline-none rounded outline outline-offset-2 outline-blue-500 mb-2">
                  {task.priority}{" "}
              ...
              microdosing tousled waistcoat.
                ...
                <div className="flex min-w-[128px] flex-col gap-3">
                    <button
                  type="button"
                  onClick={() => handleDone(idx)}
                  className="inline-flex items-center justify-center text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded"
                >Delete <MdDelete className="ml-3"/>
                </button>
                </div>
              </div>
            </div>
